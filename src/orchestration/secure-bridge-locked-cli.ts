...
import { acquireSecureBridgeCycleLock } from './secure-bridge-lock.js';
...
async function main() {
  ...
  const projectRoot = path.resolve(process.env.NEXUS_SECURE_BRIDGE_PROJECT_ROOT ?? process.cwd());
  const lock = await acquireSecureBridgeCycleLock({ projectRoot });
  ...
  try {
    ...
    try {
      ...
      if (cycleError) console.error(`Secure Bridge cycle also failed before lock release: ${cycleError instanceof Error ? cycleError.message : String(cycleError)}`);
      throw new Error(`Secure Bridge cycle lock release failed closed: ${message}`);
    }
  }
}
